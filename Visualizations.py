...
def plot_analytical_flow_distribution_on_grid(num_elements, mean_pred, cov_pred, flows, ground_truth,
    ...
    for i in range(num_elements):
        axs[i].plot(ranges[i], analytical_marginals[i], linestyle="dashed")
...
def plot_analytical_flow_posterior_t_distribution_on_grid(dimensions, location_np, scale_matrix, df, flows):
    ...
    for i in range(dimensions):
        axs[i].plot(ranges[i], analytical_marginals[i], linestyle="dashed")
        axs[i].plot(ranges[i], flow_marginals[i].detach().numpy())
        axs[i].set_xlabel(r'$\beta_' + str(i) + '$', fontsize=15)
        axs[i].set_ylabel("Number of Samples", fontsize=15)
...
def plot_lasso_path_variance(lambdas_sorted, q_samples_sorted):
    num_subplots = 2
    min_lambda = np.log10(lambdas_sorted[0])
    max_lambda = np.log10(lambdas_sorted[-1])
    custom_ticks = [i for i in range(q_samples_sorted[0].shape[-1] + 1)]
    custom_labels = ["" if i == 0 else r'$\beta_' + str(i - 1) + '$' for i in range(q_samples_sorted[0].shape[-1] + 1)]

    fig, axs = plt.subplots(1, num_subplots, figsize=(12, 5.5))
    axs[0].boxplot(x=q_samples_sorted[0])
    axs[0].set_xticks(custom_ticks)
    axs[0].set_xticklabels(custom_labels)
    axs[0].set_xlabel(f"$\\lambda = 10^{{{min_lambda:.2f}}}$")
    ...
    axs[1].set_xlabel(f"$\\lambda = 10^{{{max_lambda:.1f}}}$")
...
def plot_flow_ridge_path_vs_ground_truth(X, Y, lambda_sorted, q_samples_sorted, variance=1,
    ...
    alphas_ridge, coefs_ridge = execute_glmnet(X.cpu().detach().numpy(), Y.cpu().detach().numpy(), lambda_sorted,
                                               alpha=0)

    alphas_ridge = alphas_ridge * n / (variance * 2)
    x_enet_path = alphas_ridge
    y_enet_path = coefs_ridge

    plot_title = 'Ridge Regression with CNF - ' + solution_type

    plot_flow_path_vs_ground_truth(x_enet_path, y_enet_path, x_flow, y_flow, plot_title, beta_group_map={},
                                   group_lasso=False, show_markers=True)
...
def plot_flow_lasso_path_vs_ground_truth(X, Y, lambda_sorted, q_samples_sorted, variance=1,
    ...
    alphas_lasso, coefs_lasso = execute_glmnet(X.cpu().detach().numpy(), Y.cpu().detach().numpy(), lambda_sorted,
                                               alpha=1)
    ...
    plot_flow_path_vs_ground_truth(x_lasso_path, y_lasso_path, x_flow, y_flow, plot_title, beta_group_map={},
                                   group_lasso=False, show_markers=True)


def plot_flow_group_lasso_path_vs_ground_truth(X, Y, grouped_indices_list, likelihood_sigma, lambda_sorted,
    ...
    alpha = lambda_sorted * 1 / (2 * n)
    W_glasso = glasso_path(X, Y, group, np.array(alpha))
    lambda_glasso = np.array(W_glasso[4]) * (2 * n) / (1)
    ...
    plot_flow_path_vs_ground_truth(lambda_glasso, coeff_glasso, lambda_flow, coeff_flow, plot_title, beta_group_map,
                                   group_lasso=True, show_legend=False)


def plot_flow_poisson_regression_path_vs_ground_truth(X, Z, lambdas_sorted, q_samples_sorted, likelihood_sigma,
                                                      solution_type, is_ridge_posterior=True):
    n = Z.shape[0]
    x_flow = lambdas_sorted
    y_flow = q_samples_sorted

    alphas_, coefs_ridge = execute_glmnet(X, Z, lambdas_sorted, alpha=0 if is_ridge_posterior else 1, family="poisson")
    alphas_ = alphas_ * n / (likelihood_sigma * likelihood_sigma)

    plot_title = 'Poisson Ridge Lasso Regression with CNF - ' + solution_type

    plot_flow_path_vs_ground_truth(alphas_, coefs_ridge, x_flow, y_flow, plot_title, beta_group_map={},
                                   group_lasso=False)


def plot_flow_group_poisson_path_vs_ground_truth(X, Y, grouped_indices_list, lambda_sorted, q_samples_sorted,
                                                 solution_type):
    lambda_flow = lambda_sorted
    coeff_flow = q_samples_sorted
...
def plot_flow_path_vs_ground_truth(lambda_gt, coeff_gt, lambda_flow, coeff_flow, plot_title, beta_group_map,
                                   group_lasso,
                                   show_legend=True,
                                   show_markers=False):
    ...
    markers = ['o', 's', '^', 'x', '+', 'v']
    markers = list(islice(cycle(markers), dimension))
    ...
    for index in range(dimension):
        ...
        plt.plot(lambda_flow, y_value, color=color, label=r'$\beta_' + str(index) + '$',
                 marker=markers[index] if show_markers else None, markevery=200)
        plt.fill_between(lambda_flow, p5_np, p95_np, color=color, alpha=0.1)

        y_value = coeff_gt[index]
        plt.plot(lambda_gt, y_value, linestyle="dashed", color=color, label=r'$\hat{\beta}_' + str(index) + "$",
                 marker=markers[index] if show_markers else None, markevery=200)
...
def plot_flow_ridge_path_vs_ground_truth_standardized_coefficients(X, Y, lambda_sorted, q_samples_sorted,
                                                                   solution_type):
...
def plot_flow_lasso_path_vs_ground_truth_standardized_coefficients(X, Y, lambda_sorted, q_samples_sorted,
                                                                   solution_type):
    plot_title = "Lasso with CNF - Standardized Coefficients - " + solution_type
    # alphas_lasso, coefs_lasso, _ = lasso_path(X, Y, alphas=lambda_sorted)
    alphas_ridge, coefs_lasso = execute_glmnet(X.cpu().detach().numpy(), Y.cpu().detach().numpy(), lambda_sorted,
                                               alpha=1)
...
def plot_flow_group_lasso_path_vs_ground_truth_standardized_coefficients(X, Y, grouped_indices_list, lambda_sorted,
    ...
    if "poisson" in solution_type.lower():
        gglasso_coeff_estimate = np.median(q_samples_sorted, axis=1)
...
def plot_flow_poisson_path_vs_ground_truth_standardized_coefficients(X, Z, lambdas_sorted, q_samples_sorted,
                                                                     solution_type, is_ridge_posterior=True):
    alphas_, coefs_ = execute_glmnet(X, Z, lambdas_sorted, alpha=0 if is_ridge_posterior else 1, family="poisson")
    poisson_coeff_estimate = coefs_.T

    plot_title = 'Poisson CNF - ' + solution_type

    list_q_samples_sorted = [q_samples_sorted]
    plot_multiple_flow_paths_vs_ground_truth_standardized_coefficients(list_q_samples_sorted, poisson_coeff_estimate,
                                                                       plot_title, beta_group_map={},
                                                                       group_lasso=False, line_width=1.2)
...
def plot_multiple_flow_paths_vs_ground_truth_standardized_coefficients(list_q_samples_sorted, gt_coeffs, plot_title,
                                                                       beta_group_map,
    ...
    for flow_index, q_samples_sorted in enumerate(list_q_samples_sorted):
        ...
        for index in range(dimension):
            ...
            plt.plot(x_flows, y_value, linestyle=my_linestyles[flow_index], linewidth=line_width, color=color,
                     label=r'$\beta$' + str(index))
    ...
    plt.xlabel(r'$|\beta|$ / max $|\beta|$', fontsize=15)
    plt.ylabel('Coefficients', fontsize=15)
    plt.title(plot_title)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    ...
    legend_lines = []
    for i in range(len(list_q_samples_sorted)):
        legend_lines.append(Line2D([0], [0], color='black', linestyle=my_linestyles[i], linewidth=line_width))
    legend_lines.append(Line2D([0], [0], color='black', linestyle='dashed', linewidth=line_width))

    if len(list_q_samples_sorted) == 2:
        labels = [r'$\overline{\beta}$ from $\tau^2_{MAP}$', r'$\beta_{MAP}$', r'$\beta_{GGLasso}$']
    else:
        if group_lasso:
            if "poisson" in plot_title.lower():  # There is no ground truth for Group Poisson Lasso, therefore displaying only our results.
                labels = [r'$\beta_{MAP}$']
                legend_lines = legend_lines[:-1]
            else:
                labels = [r'$\beta_{MAP}$', r'$\beta_{GGLasso}$']
        else:
            labels = [r'$\beta_{MAP}$', r'$\beta_{GLMnet}$']
...
def convert_gt_to_group_norms(gt_coeffs, grouped_indices_list):
    norm_array = np.empty((gt_coeffs.shape[0], len(grouped_indices_list)))
    for group_index, indices in enumerate(grouped_indices_list):
        group = gt_coeffs[:, indices]
        norm_array[:, group_index] = np.linalg.norm(group, axis=-1)
    return norm_array


def plot_group_norms_vs_lambda(X, Y, grouped_indices_list, lambda_sorted, q_samples_sorted, title, best_lambda=None,
                               show_legend=True):
    ...
    for index in range(dimension):
        ...
        plt.plot(lambda_sorted, y_value, color=colors[index], label=r'||$\beta_{g' + str(index + 1) + '}$||')

    if best_lambda is not None:
        y_min = 0
        _, y_max = plt.gca().get_ylim()
        plt.vlines(best_lambda, y_min, y_max, linestyle='dashed', label=r'$\lambda^*$', colors='orange')

    plt.xlabel(r'$\lambda$', fontsize=15)
    plt.ylabel('Group Norm', fontsize=15)
    plt.title(plot_title)
    plt.legend()
    plt.xscale("log")
    plt.xticks(fontsize=12)
...
def plot_standardized_group_norms_of_flows_and_gglasso(X, Y, grouped_indices_list, lambda_sorted, q_samples_sorted,
                                                       title="", show_gt_legends=True, show_legend=True):
    beta_group_map = {}
    for group_index, group in enumerate(grouped_indices_list):
        for beta_index in group:
            beta_group_map[beta_index] = group_index
    ...
    W_glasso = glasso_path(X, Y, group, np.array(lambda_sorted))
    gglasso_coeff_estimate = W_glasso[1].T
    gt_norm_estimate = convert_gt_to_group_norms(gglasso_coeff_estimate, grouped_indices_list)

    dimension = len(grouped_indices_list)

    if isinstance(q_samples_sorted, list):
        list_q_samples_sorted = q_samples_sorted
    else:
        list_q_samples_sorted = [q_samples_sorted]

    plot_title = 'GLR with CNF ' + title
    colors = ['blue', 'green', 'red', 'brown', 'magenta', 'yellow', 'black', 'orange', 'purple', 'cyan']
    colors = list(islice(cycle(colors), dimension))
    my_linestyles = [
        'solid',
    ...
    for flow_index, q_samples_sorted in enumerate(list_q_samples_sorted):
        norm_flow = convert_q_to_group_norms(q_samples_sorted, grouped_indices_list)
        flow_norm_estimate = np.median(norm_flow, axis=1)
        flow_norm_lambda = np.linalg.norm(flow_norm_estimate, axis=1) / np.linalg.norm(flow_norm_estimate, axis=1).max()
        flow_norm_lambda_sort_order = flow_norm_lambda.argsort()
        x_flows = flow_norm_lambda[flow_norm_lambda_sort_order]
        y_flows = flow_norm_estimate[flow_norm_lambda_sort_order]
        for index in range(dimension):
            y_value = y_flows[:, index]
            plt.plot(x_flows, y_value, color=colors[index], linestyle=my_linestyles[flow_index],
                     label=r'||$\beta_{g' + str(index + 1) + '}$||')

    gt_lambdas = np.linalg.norm(gt_norm_estimate, axis=1) / np.linalg.norm(gt_norm_estimate, axis=1).max()
    gt_lambdas_sort_order = gt_lambdas.argsort()
    x_gt = gt_lambdas[gt_lambdas_sort_order]
    y_gt = gt_norm_estimate[gt_lambdas_sort_order]
    for index in range(dimension):
        gt_y_value = y_gt[:, index]
        if show_gt_legends:
            plt.plot(x_gt, gt_y_value, color=colors[index], linestyle='dashed',
                     label=r'||$\hat{\beta_{g' + str(index + 1) + '}}$||')
        else:
            plt.plot(x_gt, gt_y_value, color=colors[index], linestyle='dashed')

    plt.xlabel(r'$||\beta_g||$ / max $||\beta_g||$', fontsize=15)
    plt.ylabel('Group Norm', fontsize=15)
    plt.title(plot_title)
    if show_legend:
        plt.legend()
        ...
        plt.legend(legend_lines, labels, loc='upper left', bbox_to_anchor=(0, 1))

    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.tight_layout()
    plt.savefig("./figures/Group_norms_fl_gt_vs_lambda" + plot_title.lower().replace(" ", "_") + ".pdf", dpi=300)
    plt.show()
    plt.close()
    return
...
def plot_flow_group_coefficients_path_vs_ground_truth(X, Y, lambda_sorted, tau_samples_sorted, solution_type):
    ...
    plt.xlabel(r'$\lambda$', fontsize=15)
    plt.xscale('log')
    plt.ylabel('Group Variances', fontsize=15)
    plt.title('Group Lasso Regression with CNF - Group Variances Vs Lambda ' + solution_type)
    plt.legend()
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
    plt.tight_layout()
    plt.savefig("./figures/Group_Lasso_Group_Variances_Vs_Lambda" + solution_type + ".pdf")
    plt.show()
    plt.close()


def plot_log_marginal_likelihood_vs_lambda(X, Y, lambda_sorted, losses_sorted, variance, title="Distribution",
                                           grouped_indices_list=None, group_lasso=False):
    ...
    if "poisson" in title.lower():
        ...
        best_lambda_cv = cv[7]
    elif "Lasso-Regression-CNF" in title:
    ...
    plt.vlines(lambda_max_likelihood, p5.min(), means.max(), label='$\lambda* Flow$', colors='b')
    ...
    f.write(f"Best_q_selected_{label_lambda_cv}:{q_selected_cv}\n")
    f.write(f"====================================================\n")
    f.close()
...
def plot_betas_from_flows_vs_from_tau_flows(dimension, beta_samples, tau_beta_samples, title):
    ...
    for i in range(dimension):
        ...
        axes[i].set_title(r'$\beta_{' + str(i) + '}$', fontsize=15)
...
def plot_parameter_space_3d(device, flows, lambda_exp, plot_marginal=False, title=""):
    # lambda_exp = 0.0
    dimensions = 2
    sample_size = 50
    ...
    flow_inputs = torch.cat([X.reshape(-1, 1), Y.reshape(-1, 1)], dim=1).to(device)
    ...
    if plot_marginal:
        ...
        ax.plot_surface(X_np, Y_np, flow_density_reshaped_np, edgecolor='royalblue', lw=0.5, rstride=1, cstride=1,
                        alpha=0.3)
    ...
    title = title + "Lambda " + str(lambda_exp.item())

    destination_directory = "./figures/Lambda " + str(lambda_exp.item())
    ...
    filename = "Marginal " if plot_marginal else ""
    plt.savefig(destination_directory + "/3D-Density-Plot-" + filename + title + ".pdf")

    plt.show()
    plt.close()


def plot_parameter_space_3d_wth_gt(device, flows, lambda_exp, X_train, Y_train, likelihood_sigma, title=""):
    dimensions = 2
    sample_size = 50
    ...
    Y_np = Y.detach().cpu().numpy()
    flow_density_reshaped_np = flow_density_reshaped.detach().cpu().numpy()
    flow_density_reshaped_np = flow_density_reshaped_np / flow_density_reshaped_np.max()
    ax.plot_surface(X_np, Y_np, flow_density_reshaped_np, cmap='Blues', alpha=0.4, rstride=1, cstride=1, edgecolor='black')

    XTX = X_train.T @ X_train
    identity_matrix = torch.eye(X_train.shape[1]).to(device)
    lamda = (10**lambda_exp).to(device)
    mu2 = torch.inverse(XTX + lamda * identity_matrix) @ (X_train.T @ Y_train)
    sigma_squared = likelihood_sigma**2
    cov2 = sigma_squared * torch.inverse(XTX + lamda * identity_matrix)

    pos = torch.dstack((X, Y)).to(device)
    cov2_inv = torch.linalg.inv(cov2)
    diff = pos - mu2

    Z2 = (1 / (2 * torch.pi * torch.sqrt(torch.linalg.det(cov2)))) * torch.exp(
        -0.5 * torch.einsum('...i,ij,...j', diff, cov2_inv, diff)
    )
    Z2_np = Z2.detach().cpu().numpy()
    Z2_np = Z2_np / Z2_np.max()
    ax.plot_surface(X_np, Y_np, Z2_np, cmap='Reds', alpha=1, rstride=1, cstride=1, edgecolor='black')
