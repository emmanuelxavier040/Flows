...
def main():
    ...
    q_selected, flows = posterior(X_train, Z_train, X_torch, Z_torch, likelihood_sigma, grouped_indices_list, epochs,
                                  flow_sample_size, context_size,
                                  lambda_min_exp, lambda_max_exp, learning_rate, W)

    print(q_selected)
    # Utilities.evaluate_poisson_model(flows, q_selected, X_test, Z_test)

    # GroupPoissonRegressionCNF.posterior(X.detach().cpu().numpy(), Z.detach().cpu().numpy(), X_torch, Z_torch,
    #                                     likelihood_sigma,
