...
#     Ws_reshaped = Ws.unsqueeze(-1)
#     m = torch.matmul
#     likelihood_matrix = torch.eye(X.shape[0]).to(device)
#     likelihood_inv = torch.inverse(likelihood_matrix)
#     XWs = torch.matmul(X, Ws_reshaped)
#     XWs = XWs.squeeze()
#     error = Y - XWs
#     term_2 = -0.5 * (1/variance) * m(m(error, likelihood_inv), error.mT).to(device)
#     # XWs = torch.matmul(X, Ws_reshaped)
#     # XWs = XWs.squeeze()
#     # squared_errors = (Y - XWs) ** 2
#     n = Y.shape[0]
...
def posterior(X, Y, X_torch, Y_torch, likelihood_sigma, grouped_indices_list, epochs, q_sample_size,
              context_size, lambda_min_exp, lambda_max_exp, learning_rate, W, show_best_lambda=True,
              show_gt_legends=True, plot_sample_size=100):
    ...
    title = 'GLR with CNF'
    View.plot_standardized_group_norms_of_flows_and_gglasso(X, Y, grouped_indices_list, lambdas_sorted,
                                                            q_samples_sorted, title, show_gt_legends)

    if show_best_lambda:
        View.plot_group_norms_vs_lambda(X, Y, grouped_indices_list, lambdas_sorted, q_samples_sorted, title,
...
def main():
    ...
    flows, lambda_max_marginal_likelihood = posterior(X_train.detach().cpu().numpy(), Y_train.detach().cpu().numpy(),
                                                      ...
                                                      show_best_lambda=False, show_gt_legends=False, plot_sample_size=50)

    q_selected = Utilities.select_q_for_max_likelihood_lambda(lambda_max_marginal_likelihood, flows, device)
    Utilities.save_text_file("Best_parameter_GLasso.txt", str(q_selected))
    ...
    # ========================= To compare Group Lasso and Lasso regression in Special case ============================

    # flows, lambda_max_marginal_likelihood = LassoRegressionCNF.posterior(X_train, Y_train, X_torch, Y_torch, likelihood_sigma, epochs,
