...
def train(flows, d, X, Y, variance, num_iter, q_sample_size, lamda = 1):
    ...
    for i in range(num_iter):
        optimizer.zero_grad()
        # lambda_list = torch.ones(q_sample_size).view(-1, 1)
        # uniform_p = lambda_list.new_ones(q_sample_size).view(-1, 1) * lamda
        # context = torch.cat((lambda_list, uniform_p), 1)
        # q_samples, q_log_prob = flows.sample_and_log_prob(q_sample_size, context)
