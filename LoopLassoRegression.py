...
def log_prior_unnormalized(W, variance):
    ...
    log_prior = d * math.log(lmbda/std_dev) - (lmbda/std_dev)*torch.norm(W, p=1)
    return log_prior
...
# data_mean = torch.tensor([10.0, 11.2, 12.2, 1.1, 1.1, 12.1, 12.1, 120.1, 12.1, 12.1])
...
W = torch.tensor([24.0, 11.5, 12.4, 60.3, 12.2, 11.1, 15.3, 120.1, 9.0, 10.0])
...
for _ in range(num_layers):
    # InverseTransform(transforms.append(ReversePermutation(features=d)))
    # InverseTransform(transforms.append(MaskedAffineAutoregressiveTransform(features=d,
    #                                                       hidden_features=15)))
